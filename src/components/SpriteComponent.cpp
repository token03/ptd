...
void SpriteComponent::Draw() {
  ...
  if (auto transform = m_transform.lock()) {
    ...
    Vector2 scaledOrigin = {origin.x * transform->scale.x, origin.y * transform->scale.y};
    ...
    DrawTexturePro(texture, sourceRect, destRect, scaledOrigin, rotationToApply, tint);

    Rectangle actualScreenRect = {transform->position.x - scaledOrigin.x,
                                  transform->position.y - scaledOrigin.y, destRect.width,
                                  destRect.height};
